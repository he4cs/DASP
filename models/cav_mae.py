...
class Block(nn.Module):
    ...
    def forward(self, x, modality=None, return_attn=False):
        if modality is None:
            out, attn = self.attn(self.norm1(x), return_attn)
            x = x + self.drop_path(out)
            x = x + self.drop_path(self.mlp(self.norm2(x)))
        elif modality == 'a':
            out, attn = self.attn(self.norm1_a(x), return_attn)
            x = x + self.drop_path(out)
            x = x + self.drop_path(self.mlp(self.norm2_a(x)))
        elif modality == 'v':
            out, attn = self.attn(self.norm1_v(x), return_attn)
            x = x + self.drop_path(out)
...
class CAVMAEFT(nn.Module):
    def __init__(self, label_dim, img_size=224, audio_length=1024, patch_size=16, in_chans=3,
        ...
        self.blocks_a = nn.ModuleList([Block(embed_dim, num_heads, mlp_ratio, qkv_bias=True, norm_layer=norm_layer) 
                                       for i in range(modality_specific_depth)])
        self.blocks_v = nn.ModuleList([Block(embed_dim, num_heads, mlp_ratio, qkv_bias=True, norm_layer=norm_layer) 
                                       for i in range(modality_specific_depth)])
        self.blocks_u = nn.ModuleList([Block(embed_dim, num_heads, mlp_ratio, qkv_bias=True, norm_layer=norm_layer) 
    ...
    def get_patch_num(self, input_shape, stride):
        ...
        return test_output.shape[2], test_output.shape[3], test_output.shape[2] * test_output.shape[3]
    ...
    def forward(self, a, v, mode='multimodal', return_attn=False):
        ...
        if mode == 'multimodal':
            ...
            attns = []
            for blk in self.blocks_u:
                outputs = blk(x, return_attn=return_attn)
                if return_attn:
                    x, attn = outputs
                    attns.append(attn)
                else:
            ...
            if return_attn:
                return x, torch.stack(attns).mean(dim=0)
            else:
                return x
        ...
        # used in case that the model is fine-tuned with both modality, but in inference only image is given
            ...
            with torch.no_grad():
                ...
                for blk in self.blocks_u:
                    embed_a = blk(embed_a)
                embed_a = self.norm(embed_a)
                embed_a = embed_a.mean(dim=1)
                
                embed_v = v
                for blk in self.blocks_u:
                    embed_v = blk(embed_v)
                embed_v = self.norm(embed_v)
                embed_v = embed_v.mean(dim=1)
            
            return x, a, v, embed_a, embed_v